...
async fn change_camera(
...
) -> Result<SerialResponse<ChangeCameraResponse>, ErrorResponse> {
    ...
    if config.index == index {
        ...
        info!("{message}");
        return Ok(SerialResponse::new(ChangeCameraResponse { message }));
    }
    ...
    Ok(SerialResponse::new(ChangeCameraResponse { message }))
}
...
async fn main() -> Result<(), Error> {
    ...
    let camera_info_route = Route::get("/info").description("View current camera data.");

    // Route to change camera index.
    let change_camera_route = Route::get("/change-camera").description("Change camera.");

    // Route to change stream format.
    let change_stream_format_route =
        Route::post("/stream-format").description("Change stream format.");

    // Route to view camera stream.
    let camera_stream_route = Route::get("/stream").description("View camera stream.");

    // Route to take a screenshot with a random format.
    let screenshot_random_route =
