...
impl<'device> RequestInfo<'device> {
    pub(crate) fn new(route: &'device str, request_sender: &'device RequestSender) -> Self {
        Self {
            route,
            rest_kind: request_sender.kind,
            hazards: &request_sender.hazards,
            parameters_data: &request_sender.parameters_data,
            response_kind: request_sender.response_kind,
        }
    }
}
